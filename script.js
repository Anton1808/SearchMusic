...
function showSongs(data) {
    ...
    `<li class="song-item"><strong>${song.artist.name}</strong><span class="dash">-</span> 
    <span>${song.title}</span>
    <a class="listen" target="_blank" href=${song.preview}>Listen preview</a>
    </li>`)
    result.innerHTML = `<ul>${output}</ul>`
    if(data.prev || data.next){
    more.innerHTML = `${data.prev ? `<button onclick="getMoreSongs('${data.prev}')" class="btn">Prev</button>` : ``}
